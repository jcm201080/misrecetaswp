<?php get_header(); ?>
        <main class="grid txt-center">
            <section class="col-desk-9 col-tab-8 col-mob-12 separator-block">
                <div class="grid">
                    <div class="col-desk-6 col-tab-6 mb-2">
                        <h2 class="head-strong mb-05">lubina</h2>
                        <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(); ?>/resources/lubina-con-hummus-de-guisantes.jpg" alt="">
                    </div>
                    <div class="col-desk-6 col-tab-6 mb-2">
                        <h2 class="head-strong txt.center mb-05">espagueteis</h2>
                        <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(  ) ?>/resources/espaguetis-de-calabacin.jpg" alt="">
                    </div>
                    <div class="col-desk-6 col-tab-6 mb-2">
                        <h2 class="head-strong txt.center mb-05">tiramisu-express</h2>
                        <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(  ) ?>/resources/tiramisu-express.jpg" alt="">
                    </div>
                    <div class="col-desk-6 col-tab-6 mb-2">
                        <h2 class="head-strong txt.center mb-05 ">vichyssoise-de-esparragos-blancos</h2>
                        <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(  ) ?>/resources/vichyssoise-de-esparragos-blancos.jpg" alt="">
                    </div>
                </div>
            </section>

            <aside class="col-desk-3 col-tab-4 col-mob-12">
                <h2 class="head-strong mb-05">Menu del dia</h2>
                <div class="bg-light pad-1 rounded">
                    <div class="grid">
                        <div class="col-mob-4 mb-1">
                            <h3 class="head-strong mb-05">Entrante</h3>
                            <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(  ) ?>/resources/espaguetis-de-calabacin.jpg" alt="">
                        </div>
                    </div>

                    <div class="grid">
                        <div class="col-mob-4 mb-1">
                            <h3 class="head-strong  mb-05">principal</h3>
                            <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(  ) ?>/resources/vichyssoise-de-esparragos-blancos.jpg"
                                alt="">
                        </div>
                    </div>
                    <div class="grid">
                        <div class="col-mob-4 mb-1">
                            <h3 class="head-strong  mb-05">Postre</h3>
                            <img class="img-md rounded" src="<?php echo get_stylesheet_directory_uri(  ) ?>/resources/tiramisu-express.jpg" alt="">
                        </div>
                    </div>
                </div>

            </aside>
        </main>
 <?php get_footer(); ?>
   